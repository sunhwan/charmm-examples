...
class CHARMM:
    ...
    def __init__(self, output=None, exec=_charmm_exec, data=_charmm_data):
        self._pid = sp.Popen(exec, stdin=sp.PIPE, stdout=sp.PIPE, close_fds=True, universal_newlines=True)
    ...
    def loadParameters(self, top=None, par=None):
        top = "%s/%s" % (self.data, top)
        par = "%s/%s" % (self.data, par)
        self.sendCommand("""
read  rtf card name %s
read para card name %s flex""" % (top, par))
